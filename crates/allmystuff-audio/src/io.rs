...
use crate::pcm::{
    buffer_playback_samples, downmix, f32_to_i16, i16_to_f32, resample_linear, TARGET_DEPTH_MS,
};
use crate::{CaptureSource, DebugStats, StatsPolicy};
...
pub struct AudioBridge<S: StatsPolicy = DebugStats> {
    ...
    stats_policy: PhantomData<fn() -> S>,
}

impl<S: StatsPolicy> Default for AudioBridge<S> {
    fn default() -> Self {
        Self {
            captures: Mutex::default(),
            playbacks: Mutex::default(),
            stats_policy: PhantomData,
        }
    }
}

impl<S: StatsPolicy> AudioBridge<S> {
    pub fn new() -> Self {
        Self::default()
    }
        ...
        F: Fn(Vec<i16>, u32) + Send + Sync + 'static,
    {
        ...
        if self.captures.lock().contains_key(&route_id) {
            tracing::debug!(target: "allmystuff_node::audio",
        ...
        let thread = std::thread::spawn(move || {
            if let Err(e) = run_capture::<S, F>(&stop_thread, &id, source, on_frame) {
                tracing::warn!(target: "allmystuff_node::audio", "audio capture for {id} stopped: {e}");
            }
        });
    ...
    pub fn start_playback(&self, route_id: String) {
        ...
        let thread = std::thread::spawn(move || {
            ...
            {
                tracing::warn!(target: "allmystuff_node::audio", "audio playback for {id} stopped: {e}");
            }
        });
        let displaced = {
    ...
    pub fn feed(&self, route_id: &str, frame: &AudioFrame) {
        ...
        if pb.fed.fetch_add(1, Ordering::Relaxed) == 0 {
            tracing::info!(target: "allmystuff_node::audio",
        ...
        let mono = downmix(&frame.pcm, frame.channels);
        {
            ...
            stats.note(&mono, move |line| {
                stats_log::<S>(format!("audio in {id}: {line}"))
            });
        }
...
fn stats_log<S: StatsPolicy>(line: String) {
    if S::stats_to_info() {
        tracing::info!(target: "allmystuff_node::audio", "{line}");
    } else {
        tracing::debug!(target: "allmystuff_node::audio", "{line}");
    }
}

// ---- capture ----------------------------------------------------------

fn run_capture<S: StatsPolicy, F>(
    stop: &AtomicBool,
    route_id: &str,
    ...
    F: Fn(Vec<i16>, u32) + Send + Sync + 'static,
{
    ...
    let on_frame = Arc::new(metered::<S, F>(route_id, source, on_frame));
    match source {
    ...
    move |pcm: Vec<i16>, rate: u32| {
        {
            ...
            let silent = stats.note(&pcm, move |line| {
                stats_log::<S>(format!("audio out {line_id}: {line}"));
            });
            if silent && source == CaptureSource::Mic && !stats.warned_silent {
                stats.warned_silent = true;
                tracing::warn!(target: "allmystuff_node::audio",
    ...
    F: Fn(Vec<i16>, u32) + Send + Sync + 'static,
{
    ...
    let err = |e| tracing::warn!(target: "allmystuff_node::audio", "input stream error: {e}");
...
macro_rules! fill {
    ($ring:expr, $channels:expr, $data:expr, $conv:expr) => {{
...
fn run_playback(
    ...
    let err = |e| tracing::warn!(target: "allmystuff_node::audio", "output stream error: {e}");

    let stream = match fmt {
        cpal::SampleFormat::F32 => device.build_output_stream(
            &config,
            move |data: &mut [f32], _: &_| fill!(ring, channels, data, i16_to_f32),
            err,
            None,
        ),
        cpal::SampleFormat::I16 => device.build_output_stream(
            &config,
            move |data: &mut [i16], _: &_| fill!(ring, channels, data, |s| s),
            err,
            None,
        ),
...
mod tests {
    ...
    type AudioBridge = super::AudioBridge<DebugStats>;
