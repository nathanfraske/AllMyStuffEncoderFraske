...
fn oracle_stats_to_info() -> bool {
    TestStats::stats_to_info()
}
...
struct Pair {
    old: original::harness::Model,
    new: extracted::harness::Model,
}
impl Pair {
    fn new() -> Self {
        Self {
            old: original::harness::Model::new(),
            new: extracted::harness::Model::new(),
        }
    }
    fn playback(&self, id: &str, rate: u32, backed: bool) {
        self.old.playback(id, rate, backed);
        self.new.playback(id, rate, backed);
    }
    fn capture(&self, id: &str) {
        self.old.capture(id);
        self.new.capture(id);
    }
    fn feed(&self, id: &str, frame: &AudioFrame) {
        self.old.feed(id, frame);
        self.new.feed(id, frame);
    }
    fn state(&self, id: &str) -> FeedState {
        let old = self.old.feed_state(id);
        let new = self.new.feed_state(id);
        assert_eq!(new, old);
        new
    }
}
...
fn feed_missing_or_unbacked_routes_preserves_maps_and_does_nothing() {
    ...
    pair.playback("unbacked", 48000, false);
    let frame = AudioFrame::new("capture", 3, 48000, 1, vec![9]);
    for id in ["", "missing", "capture", "unbacked"] {
        pair.feed(id, &frame);
    }
...
fn feed_uses_argument_route_and_counts_empty_frames_before_processing() {
    let pair = Pair::new();
    pair.playback("r", 48000, true);
    pair.feed(
        "r",
        &AudioFrame::new("different", u64::MAX, 0, 0, vec![3, -4]),
    );
...
fn feed_observes_device_rate_changes_and_original_downmix_resample_order() {
    ...
    pair.feed(
        "r",
        &AudioFrame::new("r", 0, 24000, 2, vec![0, 20, 20, 40, 50]),
    );
    assert_eq!(pair.state("r").ring, [10, 20, 30, 40, 50, 50]);
    pair.old.set_rate("r", 24000);
    pair.new.set_rate("r", 24000);
    pair.feed(
        "r",
        &AudioFrame::new("r", 0, 48000, 1, vec![100, 200, 300, 400]),
    );
    pair.feed("r", &AudioFrame::new("r", 0, 0, 1, vec![500, 600]));
    assert_eq!(
        pair.state("r").ring,
        [10, 20, 30, 40, 50, 50, 100, 300, 500, 600]
    );
}
...
fn feed_keeps_strict_trim_threshold_and_can_trim_on_empty_input() {
    ...
    assert_eq!(state.ring.len(), 3840);
    assert_eq!(state.ring[0], 5761);
    pair.old.set_ring("r", &[1, 2]);
    pair.new.set_ring("r", &[1, 2]);
    pair.old.set_rate("r", 999);
    pair.new.set_rate("r", 999);
...
fn duplicate_start_guards_keep_seeded_capture_and_playback_resources() {
    let pair = Pair::new();
    pair.capture("r");
    pair.playback("r", 44100, true);
    pair.feed("r", &AudioFrame::new("r", 0, 0, 1, vec![7]));
    let before = pair.state("r");
    pair.old.duplicate_starts("r");
    pair.new.duplicate_starts("r");
...
fn running_is_map_presence_even_without_a_live_worker() {
    let pair = Pair::new();
    pair.capture("capture");
    pair.playback("playback", 0, false);
    for id in ["capture", "playback"] {
        assert!(pair.old.running(id));
        assert!(pair.new.running(id));
    }
    assert!(!pair.old.running("missing"));
    assert!(!pair.new.running("missing"));
    pair.old.stop("capture");
    pair.new.stop("capture");
    assert!(!pair.old.running("capture"));
    assert!(!pair.new.running("capture"));
    assert!(pair.old.running("playback"));
    assert!(pair.new.running("playback"));
}

fn stopped(rx: Receiver<StopObservation>) -> StopObservation {
    let state = rx
        .recv_timeout(Duration::from_secs(6))
        .expect("fake worker observation");
    assert!(
        state.stopped,
        "fake worker reached its deadline without the stop signal"
    );
    state
}
    ...
    assert_eq!(
        ...
        [
            StopObservation {
                stopped: true,
                other_stopped: false,
                capture_present: Some(false),
                playback_present: Some(false)
            },
            StopObservation {
                stopped: true,
                other_stopped: true,
    ...
    assert_eq!(
        ...
        [
            StopObservation {
                ...
                playback_present: Some(true)
            },
            StopObservation {
                stopped: true,
                other_stopped: true,
                capture_present: Some(false),
                playback_present: None
            },
        ]
    );
}
...
fn level_window_saturates_the_signed_minimum_and_resets_only_window_fields() {
    ...
    old.warned();
    new.warned();
    let first = new.note(&[i16::MIN, 2]);
    assert_eq!(first, old.note(&[i16::MIN, 2]));
    assert_eq!(
        first,
        WindowState {
            silent: false,
    ...
    new.age();
    let next = new.note(&[]);
    assert_eq!(next, old.note(&[]));
    assert_eq!(
        next,
        WindowState {
            silent: false,
            lines: vec!["0 buffers/s · peak 100%".into()],
            frames: 0,
            peak: 0,
            warned: true
        }
...
fn level_window_counts_empty_buffers_and_reports_silence_without_marking_warning() {
    ...
    old.age();
    new.age();
    let state = new.note(&[0]);
    assert_eq!(state, old.note(&[0]));
    assert_eq!(
        state,
        WindowState {
            silent: true,
            lines: vec!["0 buffers/s · peak 0%".into()],
            frames: 0,
            peak: 0,
            warned: false
        }
...
fn output_callback_pops_once_per_frame_including_partial_interleaved_tail() {
    ...
    for channels in [1, 2, 3, 8] {
        for len in [0, 1, 2, 5, 9, 40] {
            assert_eq!(
                extracted::harness::output_i16(&input, channels, len),
                original::harness::output_i16(&input, channels, len)
            );
        }
    }
    assert_eq!(
        extracted::harness::output_i16(&input, 2, 5),
        (vec![10, 10, -20, -20, 30], vec![40])
    );
}
...
fn output_callback_preserves_float_unsigned_and_underrun_silence_values() {
    ...
    assert_eq!(
        extracted::harness::output_f32(&input, 2, 8),
        original::harness::output_f32(&input, 2, 8)
    );
    assert_eq!(
        extracted::harness::output_u16(&input, 2, 8),
        original::harness::output_u16(&input, 2, 8)
    );
    assert_eq!(
        extracted::harness::output_f32(&input, 2, 8).0,
        [
            -1.0,
            -1.0,
            0.0,
    ...
    assert_eq!(
        extracted::harness::output_u16(&input, 2, 8).0,
        [0, 0, 32768, 32768, 65535, 65535, 32768, 32768]
    );
}
...
fn output_callback_preserves_zero_channel_panic_even_for_empty_output() {
    for len in [0, 1] {
        assert!(
            catch_unwind(AssertUnwindSafe(|| original::harness::output_i16(
                &[1],
                0,
                len
            )))
        ...
        assert!(
            catch_unwind(AssertUnwindSafe(|| extracted::harness::output_i16(
                &[1],
                0,
                len
            )))
...
fn statistics_policy_is_lazy_at_the_original_emission_points() {
    ...
    pair.old.age_feed_stats("r");
    pair.new.age_feed_stats("r");
    pair.feed("r", &AudioFrame::new("r", 1, 48000, 1, vec![2]));
    POLICY.with(|p| assert_eq!(p.get(), (true, 2)));
    original::harness::log_line();
    extracted::harness::log_line();
