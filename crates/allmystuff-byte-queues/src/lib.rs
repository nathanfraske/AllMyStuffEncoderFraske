...
//! never heals.
//!
//! Each key has one queue. A new watcher takes ownership of its buffered
//! chunks; tokens only scope `unwatch`, not access permission. The byte
//! budget counts payload bytes per key, excluding framing and allocations.
//! Overflow drops whole oldest chunks, including an oversized new chunk
//! if necessary. It does not bound the number of keys or empty chunks.
//!
//! [`ByteQueues::enqueue`] reports whether the queue was empty before the
//! append. An oversized chunk can therefore return `true` even though the
//! queue is empty again after eviction. Callers should treat this as a cue
//! to poll, not an acknowledgement of delivery.
//!
//! [`ByteQueues::poll`] drains `[u32 little-endian length][chunk bytes]`
//! records for local IPC. Callers must keep chunk lengths representable as
//! `u32`; this library does not validate that limit or supply reliable
//! transport, Mesh admission, or an aggregate resource budget.
//!
//! ```
//! use allmystuff_byte_queues::ByteQueues;
//!
//! let queues = ByteQueues::new(1024);
//! queues.ensure("terminal");
//! queues.enqueue("terminal", b"hi".to_vec());
//! let token = queues.watch("terminal");
//! assert_eq!(queues.poll("terminal"), [2, 0, 0, 0, b'h', b'i']);
//! queues.unwatch("terminal", token);
//! ```
...
impl ByteQueues {
    ...
    /// Buffer one inbound chunk for the watching window. Returns whether
    /// the queue was empty before appending: a hint to poll, not a delivery
    /// acknowledgement. Overflow can leave it empty even when this is `true`.
