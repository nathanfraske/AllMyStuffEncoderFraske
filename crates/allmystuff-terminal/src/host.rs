...
pub struct TerminalHost<S: TaskSpawner> {
    sessions: Sessions,
    ...
    output: ByteQueues,
    spawner: std::marker::PhantomData<fn() -> S>,
}

impl<S: TaskSpawner> Default for TerminalHost<S> {
    fn default() -> Self {
        Self::new()
    }
}

impl<S: TaskSpawner> TerminalHost<S> {
    pub fn new() -> Self {
        TerminalHost {
    ...
    fn open_with(
        ...
        for cmd in candidates {
            ...
            match pair.slave.spawn_command(cmd) {
                ...
                Err(e) => {
                    tracing::debug!(target: "allmystuff_node::terminal", "terminal shell candidate {label} failed: {e:#}");
                    last_err = format!("{e:#}");
                }
            }
        }
        ...
        spawn_named(&format!("amst-term-read {sid_r}"), move || {
            ...
            loop {
                match reader.read(&mut buf) {
                    ...
                    Err(e) => {
                        tracing::warn!(target: "allmystuff_node::terminal", session = %sid_r, error = %e, "terminal PTY reader stopped");
                        break;
                    }
                }
            }
                ...
                match msg {
                    CtlMsg::Data(bytes) => {
                        ...
                        if let Err(e) = writer.write_all(&bytes).and_then(|()| writer.flush()) {
                            ...
                            tracing::warn!(
                                target: "allmystuff_node::terminal",
                                session = %sid_c,
                                error = %e,
        ...
        S::spawn(async move {
            ...
            if still_idle {
                if let Some(mut s) = map.remove(&session_id) {
                    tracing::info!(target: "allmystuff_node::terminal", "terminal session {session_id} reaped (idle)");
                    let _ = s.killer.kill();
                    let _ = s.ctl_tx.try_send(CtlMsg::Shutdown);
                }
            }
    ...
    pub fn resize(&self, route_id: &str, cols: u16, rows: u16) -> bool {
        ...
        if s.reconcile() {
            ...
            tracing::warn!(target: "allmystuff_node::terminal", "terminal {route_id}: resize dropped (shell not draining)");
            false
        }
    }
    ...
    fn ctl_send(&self, route_id: &str, msg: CtlMsg) -> bool {
        ...
        match s.ctl_tx.try_send(msg) {
            ...
            Err(std::sync::mpsc::TrySendError::Full(_)) => {
                tracing::warn!(target: "allmystuff_node::terminal", "terminal {route_id}: input dropped (shell input queue full)");
                false
            }
            Err(std::sync::mpsc::TrySendError::Disconnected(_)) => {
                tracing::warn!(target: "allmystuff_node::terminal", "terminal {route_id}: input refused (PTY writer stopped)");
                false
            }
        }
    }
...
fn bridge_to_mpsc<S: TaskSpawner>(attach: TermAttach) -> tokio::sync::mpsc::Receiver<OutMsg> {
    let TermAttach {
        scrollback, mut rx, ..
    } = attach;
...
fn spawn_named(name: &str, f: impl FnOnce() + Send + 'static) {
    if let Err(e) = std::thread::Builder::new().name(name.to_string()).spawn(f) {
        tracing::error!(target: "allmystuff_node::terminal", thread = name, error = %e, "couldn't start terminal worker thread");
    }
}
...
mod tests {
    ...
    type TerminalHost = super::TerminalHost<crate::test_runtime::Spawner>;

    fn bridge_to_mpsc(attach: TermAttach) -> tokio::sync::mpsc::Receiver<OutMsg> {
        super::bridge_to_mpsc::<crate::test_runtime::Spawner>(attach)
    }
