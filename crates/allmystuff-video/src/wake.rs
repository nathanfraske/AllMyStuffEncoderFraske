        ...
        if st.holders == 1 {
            match platform::acquire(reason) {
                Ok(hold) => {
                    ...
                    tracing::info!(target: "allmystuff_node::wake", "display keep-awake held ({reason})");
                }
                Err(e) => {
                    // The guard still counts; only the OS-level hold is
                    // missing. The capture status keeps the viewer honest.
                    tracing::warn!(target: "allmystuff_node::wake", "display keep-awake unavailable: {e}");
                }
            }
        }
        DisplayAwake(())
    ...
    fn drop(&mut self) {
        ...
        if st.holders == 0 {
            if let Some(hold) = st.backend.take() {
                ...
                tracing::info!(target: "allmystuff_node::wake", "display keep-awake released");
            }
        }
    }
}
    ...
    pub fn acquire(_reason: &str) -> Result<Hold, String> {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if ok == 0 {
                    tracing::warn!(target: "allmystuff_node::wake::platform", "SetThreadExecutionState refused the keep-awake");
                }
...
mod platform {
    ...
    pub fn force_display_on() {
        ...
        if status != 0 {
            tracing::debug!(target: "allmystuff_node::wake::platform", "IOPMAssertionDeclareUserActivity refused: {status}");
        }
    }
}
...
mod platform {
    ...
    pub fn release(hold: Hold) {
        ...
        if let Err(e) = result {
            tracing::debug!(target: "allmystuff_node::wake::platform", "display keep-awake uninhibit: {e}");
        }
    }
    ...
    fn x11_dpms_on() {
        ...
        let (conn, _) = match connected {
            Ok(c) => c,
            Err(e) => {
                tracing::debug!(target: "allmystuff_node::wake::platform", "DPMS wake: no X connection: {e}");
                return;
            }
        };
        ...
        if let Err(e) = conn.check_request(cookie) {
            tracing::debug!(target: "allmystuff_node::wake::platform", "DPMS ForceLevel(On): {e}");
        }
    }
