...
//! state to desync).
//!
//! Mirrors the application's audio bridge's shape: each sourcing route runs
...
impl StreamStats {
    ...
    fn maybe_log(&mut self) {
        ...
        if stats_to_info() {
            tracing::info!(target: "allmystuff_node::video", "{line}");
        } else {
            tracing::debug!(target: "allmystuff_node::video", "{line}");
        }
...
fn env_u32_opt(key: &str) -> Option<u32> {
    match std::env::var(key) {
        Ok(v) if !v.trim().is_empty() => match v.trim().parse() {
            Ok(n) => {
                tracing::info!(target: "allmystuff_node::video", "{key}={n} (override)");
                Some(n)
            }
            Err(_) => {
                tracing::warn!(target: "allmystuff_node::video", "{key}={v} isn't a number — using the automatic dial");
                None
            }
        },
        _ => None,
...
fn env_u32(key: &str, default: u32) -> u32 {
    match std::env::var(key) {
        Ok(v) if !v.trim().is_empty() => match v.trim().parse() {
            ...
            Err(_) => {
                tracing::warn!(target: "allmystuff_node::video", "{key}={v} isn't a number — using {default}");
                default
            }
        },
        _ => default,
...
pub struct VideoBridge<D: crate::host::DesktopFollower> {
    desktop: std::marker::PhantomData<fn() -> D>,
...
impl<D: crate::host::DesktopFollower> Default for VideoBridge<D> {
    fn default() -> Self {
        Self {
            desktop: std::marker::PhantomData,
            routes: Mutex::default(),
            feedback: Mutex::default(),
            loss_marks: Mutex::default(),
        }
    }
}

impl<D: crate::host::DesktopFollower> VideoBridge<D> {
    pub fn new() -> Self {
        Self::default()
    }
        ...
        S: Fn(VideoStatusState, Option<String>) + Send + Sync + 'static,
    {
        ...
        if displaced.is_some() {
            tracing::warn!(target: "allmystuff_node::video", "replacing prior video capture worker for same-id route {route_id}");
        }
        drop(displaced);
        self.spawn_route(
            route_id,
    ...
    pub fn retune_link(&self, route_id: &str, link: LinkClass) -> bool {
        ...
        tracing::info!(target: "allmystuff_node::video",
    ...
    fn spawn_route(
        ...
        let thread = std::thread::spawn(move || {
            ...
            if let Err(e) = run_capture::<D>(
                ...
                tracing::warn!(target: "allmystuff_node::video", "{what} capture for {id} stopped: {e}");
            }
        });
        let displaced = {
    ...
    pub fn force_idr(&self, route_id: &str) {
        if let Some(r) = self.routes.lock().get(route_id) {
            ...
            tracing::debug!(target: "allmystuff_node::video", "refresh requested for {route_id}");
        }
    }
    ...
    pub fn route_wave_or_refresh(&self, route_id: &str) {
        ...
        if let Some(flag) = wave_flags().lock().get(route_id) {
            ...
            tracing::debug!(target: "allmystuff_node::video",
    ...
    pub fn note_feedback(
        ...
        let adapt = {
            ...
            if was != want {
                tracing::debug!(target: "allmystuff_node::video", "video {route_id}: forced-IDR cadence {was}ms → {want}ms");
            }
            (r.auto.clone(), r.tune)
        };
    ...
    pub fn retune(&self, route_id: &str, tune: Tune) {
        ...
        {
            tracing::debug!(target: "allmystuff_node::video", "route {route_id} retune is a no-op (same dials); keeping the capture");
            return;
        }
...
fn run_capture<D: crate::host::DesktopFollower>(
    stop: &AtomicBool,
    refresh: &Arc<AtomicBool>,
    idr_ms: &Arc<AtomicU64>,
    auto: &Arc<AutoAdapt>,
    ...
    let monitor = match select_monitor(monitor_id) {
        ...
        Err(first) => {
            ...
            match select_monitor(monitor_id) {
                Ok(m) => m,
                Err(e) => {
                    tracing::warn!(target: "allmystuff_node::video",
    ...
    #[cfg(windows)]
    {
        match monitor.id() {
            Ok(mid) => loop {
                match crate::win_capture::start_named::<D>(mid, stable_monitor_name.as_deref()) {
                    ...
                    Err(e) => tracing::warn!(target: "allmystuff_node::video",
    ...
    if wayland_session() {
        ...
        match crate::wayland_capture::open(monitor_id) {
            Ok((session, frames)) => {
                tracing::info!(target: "allmystuff_node::video", "wayland screencast session started for {route_id}");
                let result = pump_frames_with_stall(
                    stop,
                    fps,
                    &frames,
                ...
                match result {
                    ...
                    Err(e) => {
                        ...
                        if had_token && e.contains("no frame") {
                            ...
                            tracing::warn!(target: "allmystuff_node::video", "wayland screencast for {route_id} ended: {e}");
                        }
            ...
            Err(e) => {
                tracing::warn!(target: "allmystuff_node::video", "wayland screencast for {route_id} unavailable: {e}");
                reporter.report(VideoStatusState::GrabFailed, Some(e));
            }
        }
...
fn gpu_lane_preference() -> Option<bool> {
    static CHOICE: std::sync::LazyLock<Option<bool>> = std::sync::LazyLock::new(|| {
        ...
        match choice {
            Some(true) => {
                tracing::info!(target: "allmystuff_node::video", "ALLMYSTUFF_GPU_LANE=1 — experimental GPU zero-copy lane enabled")
            }
            Some(false) => {
                tracing::info!(target: "allmystuff_node::video", "ALLMYSTUFF_GPU_LANE=0 — using CPU-DXGI capture + hardware encode")
            }
            None => {}
        }
        choice
...
fn gpu_heal(
    ...
    match policy.on_error(Instant::now()) {
        ...
        PolicyVerdict::Rebuild => {
            tracing::warn!(target: "allmystuff_node::video", "GPU-lane encoder for {route_id} failed ({err}); reopening");
            match reopen() {
    ...
    let mut enc = 'open: {
        ...
        if nvenc_opt_in() {
            ...
            match crate::nvenc::NvencH264::open_on_device(
                ...
                Ok(n) => break 'open GpuCodec::Nvenc(n),
                Err(e) => tracing::info!(target: "allmystuff_node::video",
...
fn run_session_capture(
    ...
    tracing::info!(target: "allmystuff_node::video", "screen capture session started for {route_id}");
    let result = pump_frames(
        stop,
        fps,
        &frames,
        ...
        match monitor.capture_image() {
            ...
            Err(e) => {
                ...
                if failures == 1 || failures.is_multiple_of(100) {
                    tracing::warn!(target: "allmystuff_node::video", "screen grab failing for {route_id} ({failures}x): {e}");
                } else {
                    tracing::debug!(target: "allmystuff_node::video", "screen grab failed for {route_id}: {e}");
                }
                reporter.report(VideoStatusState::GrabFailed, Some(e));
            }
        }
...
fn make_encoder(
    ...
    match StreamEncoder::new(route_id, mode, source_hint, tune, refresh, idr_ms, auto) {
        Ok(enc) => Ok(enc),
        Err(e) => {
            tracing::warn!(target: "allmystuff_node::video", "encoder for {route_id} unavailable ({e}); falling back to MJPEG");
            StreamEncoder::new(
                route_id,
                VideoMode::Mjpeg,
                source_hint,
...
/// tail drops.
pub use crate::framing::PACE_SLICE_BYTES;
...
fn rate_adapt_mode() -> RateAdaptMode {
    static MODE: std::sync::LazyLock<RateAdaptMode> = std::sync::LazyLock::new(|| {
        ...
        {
            "0" | "off" | "false" => {
                tracing::info!(target: "allmystuff_node::video", "ALLMYSTUFF_RATE_ADAPT=0 — closed-loop bitrate disabled");
                RateAdaptMode::Off
            }
            "1" | "on" | "true" | "all" => {
                tracing::info!(target: "allmystuff_node::video",
                ...
                Ok(enc) => {
                    ...
                    if codec_emits_frame(&mut probe, bw, bh) {
                        ...
                        match hw.open_for_route(bw, bh, fps, bitrate, posture) {
                            ...
                            Err(e) => tracing::debug!(target: "allmystuff_node::video",
...
mod tests {
    use super::*;
    type VideoBridge = super::VideoBridge<crate::test_support::TestDesktopFollower>;
