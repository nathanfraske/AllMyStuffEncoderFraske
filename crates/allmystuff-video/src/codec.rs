//! Existing access-unit classification, not bitstream validation.
...
pub enum AuCodec {
    H264,
    Hevc,
    /// AV1 (OBU bitstream). **Decode is a STUB** — see the decoder's AV1 stub: the
...
pub fn sniff_codec(data: &[u8]) -> Option<AuCodec> {
...
/// first-frame media.
pub fn is_decode_entry(data: &[u8]) -> bool {
...
///
/// STUB status: correct enough to route the stream to the decoder's AV1 stub, which
