...
impl TimerResolutionGuard {
    pub fn hold() -> TimerResolutionGuard {
        #[cfg(windows)]
        unsafe {
...
#[cfg(windows)]
pub static MEDIA_THREADS: std::sync::Mutex<Vec<(String, isize)>> =
...
fn opt_out_process_throttling() {
    #[cfg(windows)]
    {
        ...
        ONCE.call_once(|| unsafe {
            ...
            tracing::info!(target: "allmystuff_node::os_perf",
...
fn mmcss_join() {
    #[cfg(windows)]
    {
        static WANT: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
            ...
            if on {
                tracing::info!(target: "allmystuff_node::os_perf", "ALLMYSTUFF_MMCSS on: media threads join the MMCSS Games class");
            }
            on
        });
        if !*WANT {
        ...
        if let Some(join) = *AV_SET {
            ...
            if handle == 0 {
                tracing::debug!(target: "allmystuff_node::os_perf", "MMCSS join failed for {:?}", std::thread::current().name());
            }
        }
    }
}
    ...
    #[cfg(windows)]
    {
        ...
        ONCE.call_once(|| {
            ...
            let Some(set) = set else {
                tracing::debug!(target: "allmystuff_node::os_perf", "D3DKMTSetProcessSchedulingPriorityClass unavailable");
                return;
            };
