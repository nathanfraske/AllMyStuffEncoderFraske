...
/// `HMONITOR` recycling.
pub fn start_named<D: crate::host::DesktopFollower>(
    monitor_id: u32,
    ...
    let thread = std::thread::spawn(move || pump::<D>(dup, &stop_thread, &tx));
    Ok((
        Session {
            stop,
            thread: Some(thread),
    ...
    while !stop.load(Ordering::SeqCst) {
        match dup.next_frame(100) {
            ...
            Err(NextError::AccessLost) => {
                ...
                tracing::debug!(target: "allmystuff_node::win_capture",
                ...
                if desktop.follow() {
                    tracing::info!(target: "allmystuff_node::win_capture",
                ...
                loop {
                    ...
                    match Duplication::new_named(&device_name) {
                        ...
                        Err(e) if Instant::now() < deadline => {
                            tracing::debug!(target: "allmystuff_node::win_capture", "re-acquire not ready ({e}); retrying");
                            std::thread::sleep(Duration::from_millis(250));
                        }
                        Err(e) => {
                            tracing::warn!(target: "allmystuff_node::win_capture",
...
mod gpu_lane {
    ...
    pub fn start_gpu_named(
        ...
        let adapter_luid = unsafe {
            ...
            tracing::info!(target: "allmystuff_node::win_capture::gpu_lane",
        ...
        while !stop.load(Ordering::SeqCst) {
            ...
            match d.next_gpu_frame(100, tx) {
                ...
                Err(NextError::AccessLost) => {
                    ...
                    tracing::debug!(target: "allmystuff_node::win_capture::gpu_lane",
                    ...
                    loop {
                        ...
                        match unsafe { duplicate_output_named(&d.device, &device_name) } {
                            ...
                            Err(e) if Instant::now() < deadline => {
                                tracing::debug!(target: "allmystuff_node::win_capture::gpu_lane", "GPU-lane re-acquire not ready ({e}); retrying");
                                std::thread::sleep(Duration::from_millis(250));
                            }
                            Err(e) => {
                                tracing::warn!(target: "allmystuff_node::win_capture::gpu_lane",
