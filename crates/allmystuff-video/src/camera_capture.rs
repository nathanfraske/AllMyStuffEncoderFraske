...
pub fn open(device: &str, fps: u32) -> Result<(CameraSession, Receiver<RawFrame>), String> {
    ...
    let thread = std::thread::spawn(move || {
        ...
        tracing::info!(target: "allmystuff_node::camera_capture",
            "camera {} open: {}×{} @ {} fps ({})",
            index.as_string(),
    ...
    while !stop.load(Ordering::SeqCst) {
        ...
        match camera.frame().and_then(|f| f.decode_image::<RgbAFormat>()) {
            ...
            Err(e) => {
                ...
                failures += 1;
                if failures == 1 {
                    tracing::warn!(target: "allmystuff_node::camera_capture", "camera frame failed: {e}");
                } else {
                    tracing::debug!(target: "allmystuff_node::camera_capture", "camera frame failed ({failures}x): {e}");
                }
                if failures >= 50 {
                    tracing::warn!(target: "allmystuff_node::camera_capture", "camera stream gave up after {failures} failed grabs");
                    return;
                }
...
fn resolve_index(device: &str) -> Result<CameraIndex, String> {
    ...
    if let Some(wanted) = device_ordinal(device) {
        ...
        tracing::warn!(target: "allmystuff_node::camera_capture", "camera {device} not found (unplugged?); capturing the first one instead");
    }
