//! Framing used by the existing host pipeline.

pub fn paced_slices_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        let value = std::env::var("ALLMYSTUFF_PACED_SLICES").ok();
        let on = paced_slices_requested(value.as_deref());
        if !on {
            tracing::warn!(target: "allmystuff_node::video", "ALLMYSTUFF_PACED_SLICES=0 — video burst shaping disabled");
        }
        on
    });
    *ON
...
pub fn paced_slices_requested(value: Option<&str>) -> bool {
...
pub fn paced_au_marker(chunks: usize) -> Vec<u8> {
...
pub fn paced_au_marker_count(data: &[u8]) -> Option<usize> {
    if data.len() != 26
...
/// to `max_chunk`.
pub fn split_annexb_paced(data: &[u8], max_chunk: usize) -> Vec<std::ops::Range<usize>> {
    ...
    let nals = crate::metadata::annexb_nals(data);
