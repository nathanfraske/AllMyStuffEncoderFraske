...
fn api() -> Result<&'static FunctionList, String> {
    ...
    API.get_or_init(|| unsafe {
        ...
        tracing::info!(target: "allmystuff_node::nvenc",
...
impl NvencH264 {
    ...
    unsafe fn initialize(
        ...
        let preset_guid = match requested_preset {
            ...
            Some(6 | 7) => {
                tracing::warn!(target: "allmystuff_node::nvenc",
        ...
        unsafe {
            ...
            let outcome = if status == NV_ENC_SUCCESS {
                ...
                if let Some(unlock) = self.api.unlock_bitstream {
                    ...
                    if status != NV_ENC_SUCCESS {
                        tracing::debug!(target: "allmystuff_node::nvenc", "NVENC UnlockBitstream: status {status}");
                    }
                }
            ...
            if let Some(unmap) = self.api.unmap_input_resource {
                ...
                if status != NV_ENC_SUCCESS {
                    tracing::debug!(target: "allmystuff_node::nvenc", "NVENC UnmapInputResource: status {status}");
                }
            }
            Ok(outcome)
        }
    ...
    pub fn invalidate_ref(&mut self, input_ts: u64) -> bool {
        unsafe {
            ...
            if status != NV_ENC_SUCCESS {
                tracing::debug!(target: "allmystuff_node::nvenc", "NVENC InvalidateRefFrames({input_ts}): status {status}");
                return false;
            }
            true
        }
    ...
    pub fn set_bitrate(&mut self, bitrate: u32) -> bool {
        ...
        unsafe {
            ...
            if status != NV_ENC_SUCCESS {
                tracing::debug!(target: "allmystuff_node::nvenc", "NVENC ReconfigureEncoder: status {status}");
                return false;
            }
            true
        }
