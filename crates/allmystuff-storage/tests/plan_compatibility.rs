...
fn save(state: &current::PlanState, writes: &mut Vec<String>, error: Option<&str>) -> Result<(), String> {
...
impl Pair {
    ...
    fn assert_state(&self) {
        assert_eq!(serde_json::to_string(&self.new).unwrap(), self.old.fixture_json());
    ...
    fn policy(&mut self, actor: &str, value: Value, error: Option<&str>) -> Result<Value, String> {
        ...
        let actual = current::PolicyUpdate::new(actor, convert(value.clone()))
            .and_then(|update| update.apply(&mut self.new, |state| save(state, &mut writes, error)));
        let (expected, old_writes) = old::with_save_outcome(error, || {
            self.old.set_policy(actor, convert(value))
        });
    ...
    fn allocation(
        ...
        let mut writes = Vec::new();
        let actual = current::AllocationUpdate::new(actor, device.into(), volume.into(), quota, enabled)
            .and_then(|update| update.apply(&mut self.new, |state| save(state, &mut writes, error)));
        let (expected, old_writes) = old::with_save_outcome(error, || {
            self.old.set_allocation(actor, device.into(), volume.into(), quota, enabled)
        });
        self.finish(actual, expected, writes, old_writes)
    }

    fn intent(&mut self, actor: &str, device: &str, role: &str, error: Option<&str>) -> Result<Value, String> {
        let mut writes = Vec::new();
        let actual = current::DeviceIntentUpdate::new(actor, device.into(), convert(json!(role)))
            .and_then(|update| update.apply(&mut self.new, |state| save(state, &mut writes, error)));
        let (expected, old_writes) = old::with_save_outcome(error, || {
            self.old.set_device_intent(actor, device.into(), convert(json!(role)))
        });
        self.finish(actual, expected, writes, old_writes)
    }
...
fn compare_serde<N: DeserializeOwned + Serialize, O: DeserializeOwned + Serialize>(input: &str) -> Result<String, String> {
...
fn policy_defaults_aliases_unknowns_and_exact_bytes() {
    ...
    for (input, output) in [
        ...
        (vectors["policy_alias_input"].as_str().unwrap(), vectors["policy_alias_output"].as_str().unwrap()),
        (vectors["policy_unknown_input"].as_str().unwrap(), vectors["policy_unknown_output"].as_str().unwrap()),
    ] {
        assert_eq!(compare_serde::<current::StoragePolicy, old::StoragePolicy>(input).unwrap(), output);
    }
...
fn messages_snapshots_and_persisted_state_keep_distinct_shapes() {
    ...
    for name in ["patch_default", "digest_message", "sync_request"] {
        ...
        assert_eq!(compare_serde::<current::StoragePlanMessage, old::StoragePlanMessage>(text).unwrap(), text);
    }
    assert_eq!(
        compare_serde::<current::StoragePlanMessage, old::StoragePlanMessage>(vectors["patch_legacy_input"].as_str().unwrap()).unwrap(),
        vectors["patch_default"].as_str().unwrap()
    );
    let text = vectors["snapshot_default"].as_str().unwrap();
    assert_eq!(compare_serde::<current::StoragePlanSnapshot, old::StoragePlanSnapshot>(text).unwrap(), text);
    assert_eq!(serde_json::to_string(&Pair::new().new).unwrap(), vectors["persisted_default"].as_str().unwrap());
    for role in vectors["roles"].as_array().unwrap() {
        let text = role.to_string();
        assert_eq!(compare_serde::<current::DeviceServiceRole, old::DeviceServiceRole>(&text).unwrap(), text);
    }
}
...
fn required_fields_and_type_errors_match_the_original_diagnostics() {
    for case in wire()["required_failures"].as_array().unwrap() {
        ...
        let result = match case["type"].as_str().unwrap() {
            ...
            "allocation" => compare_serde::<current::StorageAllocation, old::StorageAllocation>(text),
            "intent" => compare_serde::<current::DeviceServiceIntent, old::DeviceServiceIntent>(text),
            "snapshot" => compare_serde::<current::StoragePlanSnapshot, old::StoragePlanSnapshot>(text),
            "message" => compare_serde::<current::StoragePlanMessage, old::StoragePlanMessage>(text),
            ...
            "persisted" => {
                let actual = serde_json::from_str::<current::PlanState>(text).err().unwrap().to_string();
                let expected = old::StoragePlanStore::fixture_from_json(text, false).err().unwrap().to_string();
                assert_eq!(actual, expected);
...
fn persisted_defaults_unknown_fields_and_numeric_type_failures() {
    for text in ["{}", "{\"allocations\":{}}", "{\"deviceIntents\":{\"ignored\":9},\"future\":true}"] {
        let pair = Pair::from_json(text, false);
        assert_eq!(pair.state(), Pair::new().state());
    }
    for text in ["null", "true", "\"string\"", "{\"policy\":null}", "{\"counters\":{\"a\":-1}}", "{\"counters\":{\"a\":18446744073709551616}}"] {
        let actual = serde_json::from_str::<current::PlanState>(text).err().unwrap().to_string();
        let expected = old::StoragePlanStore::fixture_from_json(text, false).err().unwrap().to_string();
...
fn literal_compact_persisted_bytes_and_digests_match() {
    let vectors: Value = serde_json::from_str(include_str!("baseline/digest_vectors.json")).unwrap();
    for case in vectors.as_array().unwrap() {
        ...
        assert_eq!(pair.new.digest(), case["digest"].as_str().unwrap(), "{}", case["name"]);
    }
}
...
fn policy_range_endpoints_and_error_precedence_preserve_state() {
    for case in contracts()["policy_cases"].as_array().unwrap() {
        ...
        for value in case["accepted"].as_array().unwrap() {
            ...
            let mut p = policy();
            p[field] = value.clone();
            assert_eq!(pair.policy("owner", p, None).unwrap()["stamp"]["counter"], 1);
        ...
        for value in case["rejected"].as_array().unwrap() {
            ...
            let mut p = policy();
            p[field] = value.clone();
            assert_eq!(pair.policy("", p, None).unwrap_err(), "storage policy is outside its safe bounds");
...
fn resource_ids_are_utf8_byte_prefixed_and_colon_safe() {
    for case in contracts()["identity_cases"].as_array().unwrap() {
        ...
        assert_eq!(current::allocation_id(device, volume).unwrap(), case["id"].as_str().unwrap());
        assert_eq!(pair.allocation("owner", device, volume, u64::MAX, false, None).unwrap()["id"], case["id"]);
    }
    for value in ["a".repeat(512), "é".repeat(256)] {
        let mut pair = Pair::new();
        assert!(pair.allocation("owner", &value, &value, 1, true, None).is_ok());
        assert!(pair.intent("owner", &value, "automatic", None).is_ok());
    }
    for value in [String::new(), "a\0b".into(), "a".repeat(513), "é".repeat(257)] {
        let mut pair = Pair::new();
        assert_eq!(pair.allocation("", &value, "disk", 0, false, None).unwrap_err(), "invalid storage resource identity");
        assert_eq!(pair.allocation("", "desk", &value, 0, false, None).unwrap_err(), "invalid storage resource identity");
        assert_eq!(pair.intent("", &value, "automatic", None).unwrap_err(), "invalid device identity");
        assert_eq!(pair.state(), Pair::new().state());
    }
}
...
fn actors_keep_byte_bounds_and_the_existing_nul_acceptance() {
    for actor in ["x\0y".to_owned(), "a".repeat(512), "é".repeat(256)] {
        let mut pair = Pair::new();
        assert_eq!(pair.policy(&actor, policy(), None).unwrap()["stamp"]["actor"], actor);
        assert_eq!(pair.allocation(&actor, "desk", "disk", 1, true, None).unwrap()["stamp"]["counter"], 2);
        assert_eq!(pair.intent(&actor, "desk", "personal", None).unwrap()["stamp"]["counter"], 3);
    }
    for actor in [String::new(), "a".repeat(513), "é".repeat(257)] {
        let mut pair = Pair::new();
        assert_eq!(pair.policy(&actor, policy(), None).unwrap_err(), "invalid storage-plan actor");
        assert_eq!(pair.allocation(&actor, "desk", "disk", 0, true, None).unwrap_err(), "invalid storage-plan actor");
        assert_eq!(pair.intent(&actor, "desk", "automatic", None).unwrap_err(), "invalid storage-plan actor");
        assert_eq!(pair.state(), Pair::new().state());
        assert!(pair.writes.is_empty());
    }
}
...
fn invalid_quota_consumes_clock_without_record_or_save() {
    for enabled in [false, true] {
        ...
        assert_eq!(pair.allocation("owner", "desk", "disk", 0, enabled, None).unwrap_err(), "invalid storage allocation");
        assert!(pair.writes.is_empty());
        assert_eq!(pair.state()["counters"], json!({"owner": 1}));
        assert_eq!(serde_json::to_value(pair.new.snapshot()).unwrap(), initial_snapshot);
        assert_eq!(pair.new.digest(), "dbfe50029192c55d");
        assert_eq!(pair.allocation("owner", "desk", "disk", 1, enabled, None).unwrap()["stamp"]["counter"], 2);
    }
}
...
fn equal_value_setters_still_advance_and_save() {
    let mut pair = Pair::new();
    for counter in [1, 2] {
        assert_eq!(pair.policy("owner", policy(), None).unwrap()["stamp"]["counter"], counter);
        assert_eq!(pair.writes.len(), 1);
    }
    for counter in [3, 4] {
        assert_eq!(pair.allocation("second", "desk", "disk", 1, true, None).unwrap()["stamp"]["counter"], counter);
        assert_eq!(pair.writes.len(), 1);
    }
    for counter in [5, 6] {
        assert_eq!(pair.intent("owner", "desk", "alwaysOn", None).unwrap()["stamp"]["counter"], counter);
        assert_eq!(pair.writes.len(), 1);
    }
...
fn policy_merge_has_no_direct_counter_entry_but_advances_next_local_stamp() {
    let mut pair = Pair::new();
    assert!(pair.merge("relay", true, Some(policy_record(7, "remote")), vec![], vec![], None));
    assert_eq!(pair.state()["counters"], json!({}));
    assert_eq!(pair.new.digest(), "eb12e8b0aeed961e");
    assert_eq!(pair.policy("local", policy(), None).unwrap()["stamp"]["counter"], 8);
}
...
fn clock_observes_live_records_and_only_the_selected_actor_counter() {
    ...
    assert_eq!(owner.policy("owner", policy(), None).unwrap()["stamp"]["counter"], 13);
    let mut fresh = Pair::from_json(&base.to_string(), false);
    assert_eq!(fresh.policy("fresh", policy(), None).unwrap()["stamp"]["counter"], 10);
}
...
fn exhausted_clock_never_wraps_or_persists() {
    for source in ["policy", "allocation", "intent", "actor"] {
        ...
        match source {
            ...
            "allocation" => state["allocations"] = json!({"1:dv": allocation("d", "v", u64::MAX, "remote")}),
        ...
        assert_eq!(pair.policy("owner", policy(), None).unwrap_err(), "storage-plan clock exhausted");
        assert_eq!(pair.allocation("owner", "new", "disk", 1, true, None).unwrap_err(), "storage-plan clock exhausted");
        assert_eq!(pair.intent("owner", "new", "automatic", None).unwrap_err(), "storage-plan clock exhausted");
...
fn ordinary_members_require_both_device_and_original_actor() {
    for (device, actor, accepted) in [("member", "member", true), ("other", "member", false), ("member", "other", false), ("other", "other", false)] {
        let mut pair = Pair::new();
        assert_eq!(pair.merge("member", false, Some(policy_record(9, "member")), vec![allocation(device, "disk", 1, actor)], vec![intent(device, 2, actor)], None), accepted);
        assert_eq!(pair.state()["policy"]["stamp"], stamp(0, ""));
        assert_eq!(pair.new.snapshot().allocations.len(), usize::from(accepted));
        assert_eq!(pair.new.snapshot().device_intents.len(), usize::from(accepted));
    }
}
...
fn managers_relay_and_sender_precheck_is_only_nonempty() {
    for sender in ["relay".to_owned(), "x\0y".to_owned(), "s".repeat(513)] {
        let mut pair = Pair::new();
        assert!(pair.merge(&sender, true, Some(policy_record(3, "author")), vec![allocation("other", "disk", 4, "a")], vec![intent("other", 5, "b")], None));
        assert_eq!(pair.state()["counters"], json!({"a": 4, "b": 5}));
    }
    let mut pair = Pair::new();
    assert!(!pair.merge("", true, Some(policy_record(9, "author")), vec![], vec![], None));
    assert_eq!(pair.state(), Pair::new().state());
}
...
fn invalid_peer_records_are_skipped_without_counter_effects() {
    ...
    for (pointer, value) in [("/id", json!("wrong")), ("/device", json!("")), ("/volume", json!("bad\0volume")), ("/quotaBytes", json!(0)), ("/stamp/counter", json!(0)), ("/stamp/actor", json!("")), ("/stamp/actor", json!("x".repeat(513)))] {
    ...
    for (pointer, value) in [("/device", json!("")), ("/device", json!("bad\0device")), ("/stamp/counter", json!(0)), ("/stamp/actor", json!("")), ("/stamp/actor", json!("x".repeat(513)))] {
    ...
    let mut pair = Pair::new();
    assert!(!pair.merge("relay", true, Some(bad_policy), allocations, intents, Some("must not save")));
    assert_eq!(pair.state(), Pair::new().state());
    assert!(pair.writes.is_empty());
}
...
fn zero_policy_stamp_and_nul_actor_validation_match_existing_rules() {
    ...
    assert!(pair.merge("relay", true, Some(policy_record(1, "x\0y")), vec![allocation("d", "v", 2, "x\0y")], vec![intent("d", 3, "x\0y")], None));
}
...
fn equal_and_stale_stamps_preserve_first_value_and_skip_save() {
    ...
    assert!(pair.merge("relay", true, Some(p.clone()), vec![a.clone()], vec![i.clone()], None));
    let before = pair.state();
    for (counter, actor) in [(6, "z"), (7, "a"), (7, "b")] {
        ...
        assert!(!pair.merge("relay", true, Some(changed_p), vec![changed_a], vec![changed_i], Some("must not save")));
        assert_eq!(pair.state(), before);
        assert!(pair.writes.is_empty());
    }
    assert!(pair.merge("relay", true, Some(policy_record(7, "c")), vec![allocation("desk", "disk", 7, "c")], vec![intent("desk", 7, "c")], None));
}
...
fn ordered_duplicates_affect_first_value_and_historical_counters() {
    ...
    let mut pair = Pair::new();
    assert!(pair.merge("relay", true, None, vec![a.clone(), tied.clone()], vec![], None));
    ...
    assert!(forward.merge("relay", true, None, vec![a.clone(), b.clone()], vec![], None));
    let mut backward = Pair::new();
    assert!(backward.merge("relay", true, None, vec![b, a], vec![], None));
    assert_eq!(serde_json::to_value(forward.new.snapshot()).unwrap(), serde_json::to_value(backward.new.snapshot()).unwrap());
...
fn oversized_patch_rejects_policy_and_other_vectors_before_mutation() {
    for kind in ["allocations", "intents"] {
        let mut pair = Pair::new();
        let allocations = if kind == "allocations" { vec![allocation("d", "v", 1, "a"); 513] } else { vec![] };
        let intents = if kind == "intents" { vec![intent("d", 1, "a"); 513] } else { vec![] };
        assert!(!pair.merge("relay", true, Some(policy_record(9, "a")), allocations, intents, None));
        assert_eq!(pair.state(), Pair::new().state());
        assert!(pair.writes.is_empty());
    }
    let mut pair = Pair::new();
    assert!(pair.merge("relay", true, None, vec![allocation("d", "v", 1, "a"); 512], vec![intent("d", 1, "a"); 512], None));
...
fn full_maps_allow_updates_and_cap_errors_precede_actor_validation() {
    ...
    let mut pair = Pair::from_json(&state.to_string(), false);
    assert_eq!(pair.allocation("", "new", "v", 0, false, None).unwrap_err(), "the fleet storage plan has too many allocations");
    assert_eq!(pair.intent("", "new", "automatic", None).unwrap_err(), "the fleet storage plan has too many device roles");
    assert_eq!(pair.state(), state);
    assert_eq!(pair.allocation("", "d000", "v", 0, false, None).unwrap_err(), "invalid storage-plan actor");
    assert_eq!(pair.intent("", "d000", "automatic", None).unwrap_err(), "invalid storage-plan actor");
    assert!(pair.allocation("owner", "d000", "v", 2, false, None).is_ok());
...
fn last_free_slot_obeys_input_order_and_rejected_records_do_not_raise_clocks() {
    for (first, second) in [("first", "second"), ("second", "first")] {
        ...
        assert!(pair.merge("relay", true, None, vec![allocation(first, "v", 2, "a"), allocation(second, "v", 900, "rejected")], vec![intent(first, 3, "a"), intent(second, 900, "rejected")], None));
        assert_eq!(pair.new.snapshot().allocations.len(), 512);
        assert_eq!(pair.new.snapshot().device_intents.len(), 512);
        assert!(pair.new.snapshot().allocations.iter().any(|a| a.device == first));
        assert!(!pair.new.snapshot().allocations.iter().any(|a| a.device == second));
        assert_eq!(pair.state()["counters"], json!({"a": 3}));
        assert_eq!(pair.policy("owner", policy(), None).unwrap()["stamp"]["counter"], 4);
    }
}
...
fn failed_setters_restore_only_their_record_and_retries_skip_consumed_clock() {
    ...
    assert_eq!(pair.policy("owner", policy(), Some("save failed")).unwrap_err(), "save failed");
    assert_eq!(serde_json::to_value(pair.new.snapshot()).unwrap(), before);
    assert_eq!(pair.state()["counters"], json!({"owner": 1}));
    assert_eq!(pair.writes.len(), 1);
    assert_eq!(pair.policy("owner", policy(), None).unwrap()["stamp"]["counter"], 2);
    assert_eq!(pair.allocation("owner", "d", "v", 1, true, Some("save failed")).unwrap_err(), "save failed");
    assert!(pair.new.snapshot().allocations.is_empty());
    assert_eq!(pair.allocation("owner", "d", "v", 1, true, None).unwrap()["stamp"]["counter"], 4);
    let retained = pair.new.snapshot().allocations[0].clone();
    assert!(pair.allocation("owner", "d", "v", 9, false, Some("save failed")).is_err());
    assert_eq!(pair.new.snapshot().allocations, vec![retained]);
    assert_eq!(pair.intent("owner", "d", "personal", Some("save failed")).unwrap_err(), "save failed");
    assert!(pair.new.snapshot().device_intents.is_empty());
    assert_eq!(pair.intent("owner", "d", "personal", None).unwrap()["stamp"]["counter"], 7);
    let retained = pair.new.snapshot().device_intents[0].clone();
    assert!(pair.intent("owner", "d", "alwaysOn", Some("save failed")).is_err());
...
fn failed_merge_restores_full_state_and_noop_never_calls_persistence() {
    ...
    assert!(!pair.merge("relay", true, Some(p.clone()), vec![a.clone()], vec![i.clone()], Some("failed merge")));
    assert_eq!(pair.state(), before);
    ...
    assert!(pair.merge("relay", true, Some(p.clone()), vec![a.clone()], vec![i.clone()], None));
    assert!(!pair.merge("relay", true, Some(p), vec![a], vec![i], Some("must not save")));
    assert!(pair.writes.is_empty());
...
fn prepared_updates_take_the_clock_from_apply_time_state() {
    ...
    assert!(pair.merge("relay", true, Some(policy_record(40, "remote")), vec![], vec![], None));
    let mut writes = Vec::new();
    let actual = update.apply(&mut pair.new, |state| save(state, &mut writes, None));
    let (expected, old_writes) = old::with_save_outcome(None, || pair.old.set_policy("local", old::StoragePolicy::default()));
    assert_eq!(pair.finish(actual, expected, writes, old_writes).unwrap()["stamp"]["counter"], 41);
}
