...
//!
//! These are decision helpers only: they do not fetch, stage, or apply updates.
//! The ordering is intentionally semver-like rather than strict SemVer: only
//! three numeric components are read, missing or invalid components become zero,
//! and prerelease suffixes compare lexicographically. Policy names are exact,
//! lowercase tokens for both [`ApplyPolicy::parse`] and serde.
//!
//! ```
//! use allmystuff_update_policy::{policy_allows, ApplyPolicy};
//!
//! let policy = ApplyPolicy::parse("patch").unwrap();
//! assert!(policy_allows(policy, "1.2.3", "1.2.4"));
//! assert!(!policy_allows(policy, "1.2.3", "1.3.0"));
//! ```
